...
def read_srt_files(listSrt):
    ...
    for episode in listSrt:
        ...
        words = [stemmer.stem(w.lower()) for w in tokens if w.lower() not in cachedStopWords and len(w) > 3 and w.lower().isalpha()]
    ...
    print(sorted(corpusWithTf.items(), key=operator.itemgetter(1), reverse=True))
